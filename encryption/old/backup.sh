...
function chr() { mapped=${map_to[${1}]};printf \\$(printf '%03o' ${mapped}); }
function ord() { first=`printf '%d' "'$1"`; mapped=${map_from[${first}]}; echo $mapped; }
function binary { values=(128 64 32 16 8 4 2 1); holder=${1}; for i in ${values[@]}; do if [[ ${holder} -ge ${i} ]]; then holder=$((${holder}-${i})); binary+=1; else binary+=0; fi; done; echo $binary; }
function int { int=0; count=0; values=(128 64 32 16 8 4 2 1); holder=$1; for i in `echo ${holder} | grep -o .`; do if [[ $i == 1 ]]; then int=$((${int}+${values[$count]})); fi; ((count++)); done; echo ${int}; }
#testing functions
############################################################################################################################################################################################################################################################################
echo -e "\n##############################################"
echo -e "chr() test... calling \`chr 97\` == `chr 97`"
echo -e "ord() test... calling \`ord a \` == `ord a` "
echo -e "##############################################"
echo -e "\n##############################################"
...
for i in `echo $password | grep -o .`; do password_int_array+=(`ord "${i}"`); done
